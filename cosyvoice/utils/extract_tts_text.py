...
# 生成sft测试集的wav2tts_text
def extract_test_sft():
...
# 生成zero_shot测试集的wav2tts_text
def extract_test_zeroshot():
    test_reject_dir = Path("/home/Speech-Emotion-Recognition/cosyvoice/casia_dpo_2/test/reject")
    for entry in os.listdir(test_reject_dir):
        if ".txt" in entry:
            audio_name = entry.split('.')[0]
            with open(f"{test_reject_dir}/{entry}", "r", encoding="utf-8") as f:
                trans = f.readline().strip()
            output_dict[audio_name] = [trans.split('<|endofprompt|>')[1]]

    target_path = root_dir / f"wav2tts_text_dpo_1200_test_zeroshot.json"
    with open(f"{target_path}", "w", encoding="utf-8") as f:
        json.dump(output_dict, f, ensure_ascii=False, indent=2)
